...
class LCNDSTModel(BasePCNModel):
    """LCNDST Model based on BasePCNModel."""
...
class LCNDST(MOAgent, MOPolicy):
    ...
    def _act(self, obs: np.ndarray, desired_return, desired_horizon, action_mask, eval_mode=False) -> int:
        ...
        prediction = th.nn.functional.log_softmax(probs.cpu(), dim=-1)
    ...
    def _run_episode(self, env, desired_return, desired_horizon, max_return, starting_loc=None, eval_mode=False):
        ...
        while not done:
            action = self._act(obs, desired_return, desired_horizon, None, eval_mode=eval_mode)
