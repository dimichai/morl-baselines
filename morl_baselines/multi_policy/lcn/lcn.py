...
def lorenz_distance(points):
    """Find the Lorenz optimal point and determine the distance between this point and all other points."""
    # sort points per dimension
    lv = lorenz_vector(points)
    non_dominated_lv = get_non_dominated_inds(lv)
    # Get the best non-dominated lorenz point (optimal Lorenz point)
    lv = lv[non_dominated_lv][-1]
...
class LCNTNDP(MOAgent, MOPolicy):
        ...
        while self.global_step < total_timesteps:
            ...
            if self.global_step >= (n_checkpoints + 1) * total_timesteps / 100:
                ...
                if e_returns.shape[1] == 2:
                    ...
                    ax.scatter(returns[non_dominated_lv, 0], returns[non_dominated_lv, 1], marker='+', color='black', alpha=0.5, label='Non-Lorenz-dominated in ER')
                ...
                if e_returns.shape[1] == 2:
                    ...
                    # Plot the lorenz vectors of the experience replay
                    lr_er_returns = lorenz_vector(er_returns)
                    fig, ax = plt.subplots(figsize=(5, 5))
                    ax.scatter(lr_er_returns[:, 0], lr_er_returns[:, 1], alpha=0.5)
                    ax.set_xlim(pf_plot_limits)
                    ax.set_ylim(pf_plot_limits)
                    ax.set_xlabel("Objective 1")
                    ax.set_ylabel("Objective 2")
                    ax.set_title(f"Current Experience Replay {n_checkpoints}")
                    fig.legend(loc='upper left')
                    fig.savefig(f"{save_dir}/LV_ER_{n_checkpoints}.png")
                    plt.close()
