...
class QNet(nn.Module):
    ...
    def forward(self, obs, w, action_mask=None):
        ...
        return q_values - 10e10 * (1-action_mask.int().unsqueeze(-1))
...
class GPIPD(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        if self.per:
            ...
            self.replay_buffer = ReplayBuffer(
                self.observation_shape, 1, rew_dim=self.reward_dim, max_size=buffer_size, action_dtype=np.uint8, action_mask_dim=action_mask_dim
            )
    ...
    def update(self, weight: th.Tensor):
        ...
        for g in range(self.gradient_updates if self.global_step >= self.dynamics_rollout_starts else 1):
            if self.per:
                ...
                s_obs, s_actions, s_rewards, s_next_obs, s_dones, s_action_masks = self._sample_batch_experiences()
            ...
            with th.no_grad():
                ...
                next_q_values = th.stack([target_psi_net(s_next_obs, w, s_action_masks) for target_psi_net in self.target_q_nets])
            ...
            for psi_net in self.q_nets:
                psi_value = psi_net(s_obs, w, s_action_masks)
    ...
    def gpi_action(self, obs: th.Tensor, w: th.Tensor, return_policy_index=False, include_w=False, action_mask=None):
        ...
        max_q, a = th.max(scalar_q_values - 10e10 * (1-th.tensor(action_mask).int().to(self.device)), dim=1)
    ...
    def _act(self, obs: th.Tensor, w: th.Tensor, action_mask=None) -> int:
        if self.np_random.random() < self.epsilon:
            ...
            if self.use_gpi:
                action, policy_index = self.gpi_action(obs, w, return_policy_index=True, action_mask=action_mask)
                self.police_indices.append(policy_index)
                return action
            else:
                return self.max_action(obs, w, action_mask=action_mask)

    @th.no_grad()
    def max_action(self, obs: th.Tensor, w: th.Tensor, action_mask=None) -> int:
        ...
        max_act = th.argmax(q - 10e10 * (1-action_mask.int()), dim=1)
    ...
    def train(
        ...
        for iter in range(1, max_iter + 1):
            ...
            if weight_selection_algo == "gpi-ls":
                ...
                M = None
            print(f'Training iteration {iter} of {max_iter}, training using weight {w}.')
